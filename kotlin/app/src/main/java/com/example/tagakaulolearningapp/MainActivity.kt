...
class MainActivity : AppCompatActivity() {
    ...
    private var initialProgress: Double = 0.0
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        lessonText = findViewById(R.id.txtLesson)
    ...
    private fun loadResources(tableName: String) {
        ...
        lessonText.text = currentProgress.toString() + "%"
            ...
            "tbl_batch" -> {
                ...
                client.newCall(request).enqueue(object : Callback {
                    override fun onResponse(call: Call, response: Response) {
                        ...
                        runOnUiThread {
                            initialProgress += 9.09
                            lessonText.text = "%.2f%%".format(initialProgress)
                            progressBar.progress = "%.2f".format(initialProgress).toFloat().toInt()
                        }
                    }
                ...
                client.newCall(request).enqueue(object : Callback {
                    override fun onResponse(call: Call, response: Response) {
                        ...
                        runOnUiThread {
                            ...
                            if (taskInProgress) {
                                Toast.makeText(this@MainActivity, "You are back online", Toast.LENGTH_SHORT).show()
                                val intent = Intent(this@MainActivity, UserOption::class.java)
                                Handler().postDelayed({
                                    startActivity(intent)
                                    finish()
                                }, 3000)
                            } else {
                                Toast.makeText(this@MainActivity, "Failed to download resources", Toast.LENGTH_SHORT).show()
                                val intent = Intent(this@MainActivity, UserOption::class.java)
                                startActivity(intent)
                                finish()
                            }
        ...
        if (mediaPath != "") {
            ...
            if (videoCacheFile.exists()) {
                ...
                client.newCall(request).enqueue(object : Callback {
                    ...
                    override fun onFailure(call: Call, e: IOException) {
                        runOnUiThread {
                            Toast.makeText(this@MainActivity, "Unable to download audio", Toast.LENGTH_SHORT).show()
                        }
                    }
                })
    ...
    private fun insertGradeRecords(studentName: String, subjectId: String, score: Double) {
        ...
        val subjectQuery = "SELECT * FROM tbl_subject WHERE subjectId = ?"
        ...
        if (gradingCursor.count > 0) {
            ...
            val lessonArray = arrayOf("0", "Quiz", score, studentName, subject)
            db.execSQL(insertQuery, lessonArray)
            gradingCursor.close()
        } else {
            val insertQuery = "INSERT INTO $tableName (batch, studentName, quarter, record, subject, score, addedBy) VALUES (?,?,?,?,?,?,?)"
            val lessonArray = arrayOf(studentName, "0", "Quiz", subject, score, "SYS")
            db.execSQL(insertQuery, lessonArray)
            gradingCursor.close()
        }
        println("Added Existing Quiz Progress to Grade: $studentName, $score, $subject")
    }
    ...
    private fun createLearnerQuizProgress() {
        val client = OkHttpClient()
        val baseUrl = ConnectionClass()
        val fileAccess = "getLearnerQuiz.php"
    ...
    private fun createGradeRecords() {
        val client = OkHttpClient()
        val baseUrl = ConnectionClass()
        val fileAccess = "getLearnerQuiz.php"
        val url = baseUrl + fileAccess
    ...
    private fun createLearnerProgress() {
        ...
        println("Created Learner Tables")
    }
    private fun createTeacherProgress() {
        ...
        println("Created Teacher Tables")
    }
    private fun getLesson(initialProgress: Int) {
        ...
        if (lessonCursor.count > 0) {
            lessonText.text = initialProgress.toString() + "%"
            progressBar.progress = initialProgress
            println("Previous Lesson: Loaded")
            println("Offline Access: Allow")
        } else {
            println("Offline Access: Deny")
        }
    }
    ...
    private fun getLanguage(initialProgress: Int) {
        ...
        if (lessonCursor.count > 0) {
            lessonText.text = initialProgress.toString() + "%"
            progressBar.progress = initialProgress
            println("Previous Language: Loaded")
            println("Offline Access: Allow")
        } else {
            println("Offline Access: Deny")
        }
    ...
    private fun getGame(initialProgress: Int) {
        ...
        if (gameCursor.count > 0) {
            lessonText.text = initialProgress.toString() + "%"
            progressBar.progress = initialProgress
            println("Previous Accounts: Loaded")
            println("Offline Access: Allow")
        } else {
            println("Offline Access: Deny")
        }
